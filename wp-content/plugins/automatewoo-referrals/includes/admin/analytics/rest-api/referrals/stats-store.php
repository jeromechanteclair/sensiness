...
/**
 * REST API AW Referrals report stats data store
 *
 * @since 2.7.0
 */
class Stats_Store extends Generic_Stats_Store {
	...
	/**
	 * Table used to get referrals data details.
	 *
	 ...
	 * @var string
	 */
	protected $cache_key = 'referrals_stats';
	 ...
	 * @var string
	 */
	protected $context = 'referrals_stats';
	...
	public function get_noncached_data( $query_args, $params, &$data, $expected_interval_count ) {
		...
		if ( null === $totals ) {
			return new WP_Error( 'automatewoo_referrals_result_failed', __( 'Sorry, fetching referrals data failed.', 'automatewoo-referrals' ) );
		}
	...
	protected function initialize_queries() {
		...
		$join_clause            = "JOIN {$this->meta_table->name} ON ( {$order_stats_table_name}.order_id = {$this->meta_table->name}.{$this->meta_table->id_column} AND {$this->meta_table->name}.meta_key = '_aw_referral_id' )";
