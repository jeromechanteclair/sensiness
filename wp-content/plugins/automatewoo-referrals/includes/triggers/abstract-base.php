...
abstract class Trigger_Abstract extends AutomateWoo\Trigger {
	...
	function get_referral_data_layer( $referral, $primary_customer = 'friend' ) {
		...
		if ( 'friend' === $primary_customer ) {
			// if using signups referrals we don't have any order data
			$customer = AW_Referrals()->options()->get_reward_event() === 'signup' ? Customer_Factory::get_by_user_id( $referral->get_user_id() ) : Customer_Factory::get_by_order( $referral->get_order() );
		} else {
			$customer = Customer_Factory::get_by_user_id( $referral->get_advocate_id() );
		}
