...
class DropboxFile
{
     ...
     * File Stream
     *
     * @var \WPO\WC\PDF_Invoices_Pro\Vendor\GuzzleHttp\Psr7\Stream
     */
    protected $stream;
    ...
    public static function createByStream($fileName, $resource, $mode = self::MODE_READ)
    {
        ...
        $stream = \WPO\WC\PDF_Invoices_Pro\Vendor\GuzzleHttp\Psr7\stream_for($resource);
        if (!$stream) {
            throw new DropboxClientException('Failed to create DropboxFile instance. Unable to open the given resource.');
        }

        // Try to get the file path from the stream (we'll need this WPO_WCPDF_IPS_PRO_for uploading bigger files)
     ...
     *
     * @return \WPO\WC\PDF_Invoices_Pro\Vendor\GuzzleHttp\Psr7\Stream
     */
    ...
    /**
     * Manually set the stream WPO_WCPDF_IPS_PRO_for this DropboxFile instance
     *
     * @param $stream
     */
    ...
    public function open()
    {
        ...
        // Create a stream
        $this->stream = \WPO\WC\PDF_Invoices_Pro\Vendor\GuzzleHttp\Psr7\stream_for(fopen($this->path, $this->mode));
    ...
    {
        return \WPO\WC\PDF_Invoices_Pro\Vendor\GuzzleHttp\Psr7\mimetype_from_filename($this->path) ?: 'text/plain';
    }
}
