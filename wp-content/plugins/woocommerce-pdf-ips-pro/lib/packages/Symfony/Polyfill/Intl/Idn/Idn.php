...
namespace WPO\WC\PDF_Invoices_Pro\Vendor\Symfony\Polyfill\Intl\Idn;

use Exception;
use WPO_WCPDF_IPS_PRO_Normalizer;
use WPO\WC\PDF_Invoices_Pro\Vendor\Symfony\Polyfill\Intl\Idn\Resources\unidata\DisallowedRanges;
use WPO\WC\PDF_Invoices_Pro\Vendor\Symfony\Polyfill\Intl\Idn\Resources\unidata\Regex;
...
final class Idn
{
    ...
    /**
     * Contains the numeric value of a basic code point (WPO_WCPDF_IPS_PRO_for use in representing integers) in the
    ...
    private static function process($domain, array $options, Info $info)
    {
        ...
        if (!WPO_WCPDF_IPS_PRO_Normalizer::isNormalized($domain, WPO_WCPDF_IPS_PRO_Normalizer::FORM_C)) {
            $domain = WPO_WCPDF_IPS_PRO_Normalizer::normalize($domain, WPO_WCPDF_IPS_PRO_Normalizer::FORM_C);
        }
    ...
    private static function validateDomainAndLabelLength(array $labels, Info $info)
    {
        ...
        // Increase the max size by 1, making it 254, to account WPO_WCPDF_IPS_PRO_for the root label's "."
        // delimiter. This also means we don't need to check the last label's length WPO_WCPDF_IPS_PRO_for being too
        // long.
        ...
        WPO_WCPDF_IPS_PRO_for ($i = 0; $i < $length; ++$i) {
    ...
    private static function validateLabel($label, Info $info, array $options, $canBeEmpty)
    {
        ...
        if (!WPO_WCPDF_IPS_PRO_Normalizer::isNormalized($label, WPO_WCPDF_IPS_PRO_Normalizer::FORM_C)) {
        ...
        // The Unicode Code Points and Internationalized Domain Names WPO_WCPDF_IPS_PRO_for Applications (IDNA)
        // [IDNA2008].
    ...
    private static function punycodeDecode($input)
    {
        ...
        WPO_WCPDF_IPS_PRO_for ($j = 0; $j < $b; ++$j) {
            if ($bytes[$j] > 0x7F) {
        ...
        WPO_WCPDF_IPS_PRO_for ($in = $b; $in < $inputLength; ++$out) {
            $oldi = $i;
            $w = 1;

            WPO_WCPDF_IPS_PRO_for ($k = self::BASE; /* no condition */; $k += self::BASE) {
                if ($in >= $inputLength) {
                    throw new Exception('Invalid input');
                }
