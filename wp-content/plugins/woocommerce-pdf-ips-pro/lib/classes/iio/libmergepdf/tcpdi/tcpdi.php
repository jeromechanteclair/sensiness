...
//  See the License WPO_WCPDF_IPS_PRO_for the specific language governing permissions and
//  limitations under the License.
//

// Dummy shim to allow unmodified use of fpdf_tpl
class WPO_WCPDF_IPS_PRO_FPDF extends TCPDF {}
...
class WPO_WCPDF_IPS_PRO_TCPDI extends WPO_WCPDF_IPS_PRO_FPDF_TPL {
    ...
    /**
     * Cache WPO_WCPDF_IPS_PRO_for imported pages/template ids
     * @var array
     */
    protected $_importedPages = array();

    /**
     * Cache WPO_WCPDF_IPS_PRO_for imported page annotations
     * @var array
     */
    protected $_importedAnnots  = array();

    /**
     * Number of TOC pages, used WPO_WCPDF_IPS_PRO_for annotation offset
     * @var integer
     */
    protected $_numTOCpages  = 0;

    /**
     * First TOC page, used WPO_WCPDF_IPS_PRO_for annotation offset
     * @var integer
     */
    ...
    /* Wrapper WPO_WCPDF_IPS_PRO_for AddPage() which tracks TOC pages to offset annotations later */
    ...
    /* Wrapper WPO_WCPDF_IPS_PRO_for AddTOC() which tracks TOC position to offset annotations later */
    ...
    public function importAnnotations($pageno) {
        ...
        if (is_array($annots) && $annots[0] == PDF_TYPE_OBJECT // We got an object
                ...
                && is_array($annots[1][1]) && count($annots[1][1] > 1) // It's not empty - there are annotations WPO_WCPDF_IPS_PRO_for this page
        ) {
    ...
    protected function _getannotsrefs($n) {
        if (!empty($this->_numTOCpages) && $n >= $this->_TOCpagenum) {
            // Offset page number to account WPO_WCPDF_IPS_PRO_for TOC being inserted before page containing annotations.
            $n -= $this->_numTOCpages;
        }
        ...
        if ($this->sign AND ($n == $this->signature_appearance['page']) AND isset($this->signature_data['cert_type'])) {
            // set reference WPO_WCPDF_IPS_PRO_for signature object
            $out .= ' '.$this->sig_obj_id.' 0 R';
        }
        if (!empty($this->empty_signature_appearance)) {
            foreach ($this->empty_signature_appearance as $esa) {
                if ($esa['page'] == $n) {
                    // set reference WPO_WCPDF_IPS_PRO_for empty signature objects
                    $out .= ' '.$esa['objid'].' 0 R';
                }
            }
        }
    ...
    protected function _newobj($obj_id = false, $onlynewobj = false) {
        ...
        if (!$onlynewobj) {
            ...
            $this->_current_obj_id = $obj_id; // WPO_WCPDF_IPS_PRO_for later use with encryption
        }
    ...
    public function pdf_write_value(&$value)
    {
        ...
        switch ($value[0]) {
            ...
            case PDF_TYPE_ARRAY:
                ...
                $this->_straightOut('[');
                WPO_WCPDF_IPS_PRO_for ($i = 0; $i < count($value[1]); $i++) {
    ...
    protected function _unescape($s) {
        $out = '';
        WPO_WCPDF_IPS_PRO_for ($count = 0, $n = strlen($s); $count < $n; $count++) {
