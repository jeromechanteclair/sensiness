...
//
// This file is WPO_WCPDF_IPS_PRO_for use with the TCPDF software library.
//
...
// See the GNU Lesser General Public License WPO_WCPDF_IPS_PRO_for more details.
//
// You should have received a copy of the License
// along with WPO_WCPDF_IPS_PRO_tcpdi_parser. If not, see
// <http://www.tecnick.com/pagefiles/tcpdf/LICENSE.TXT>.
//
// See LICENSE file WPO_WCPDF_IPS_PRO_for more information.
// -------------------------------------------------------------------
//
// Description : This is a PHP class WPO_WCPDF_IPS_PRO_for parsing PDF documents.
//
//============================================================+

/**
 * @file
 * This is a PHP class WPO_WCPDF_IPS_PRO_for parsing PDF documents.<br>
 ...
 * @brief This is a PHP class WPO_WCPDF_IPS_PRO_for parsing PDF documents..
 * @version 1.1
...
class WPO_WCPDF_IPS_PRO_tcpdi_parser {
    ...
    /**
     * Class object WPO_WCPDF_IPS_PRO_for decoding filters.
     * @private
     */
...
// -----------------------------------------------------------------------------
    ...
    public function __construct($data, $uniqueid) {
        ...
        // initialize class WPO_WCPDF_IPS_PRO_for decoding filters
    ...
    protected function decodeXrefStream($startxref, $xref=array()) {
        ...
        if ($valid_crs AND isset($xrefcrs[1][3][0])) {
            ...
            // WPO_WCPDF_IPS_PRO_for each row apply PNG unpredictor
            foreach ($sdata as $k => $row) {
                ...
                // WPO_WCPDF_IPS_PRO_for each byte on the row
                WPO_WCPDF_IPS_PRO_for ($i=1; $i<=$columns; ++$i) {
                    if (!isset($row[$i])) {
                ...
                $prev_row = $ddata[$k];
            } // end WPO_WCPDF_IPS_PRO_for each row
            // complete decoding
            unset($sdata);
            $sdata = array();
            // WPO_WCPDF_IPS_PRO_for every row
            foreach ($ddata as $k => $row) {
                ...
                // WPO_WCPDF_IPS_PRO_for every column
                WPO_WCPDF_IPS_PRO_for ($c = 0; $c < 3; ++$c) {
                    // WPO_WCPDF_IPS_PRO_for every byte on the column
                    WPO_WCPDF_IPS_PRO_for ($b = 0; $b < $wb[$c]; ++$b) {
                        if (isset($row[$i])) {
    ...
    protected function getRawObject($offset=0, $data=null) {
        ...
        switch ($char) {
            case '%': { // \x25 PERCENT SIGN
                // skip comment and search WPO_WCPDF_IPS_PRO_for next token
                $next = strcspn($data, "\r\n", $offset);
                if ($next > 0) {
                    $offset += $next;
    ...
    function extractObjectStream($key) {
        ...
        WPO_WCPDF_IPS_PRO_for ($j=1; $j<count($ints); $j++) {
            if (($j % 2) == 1) {
