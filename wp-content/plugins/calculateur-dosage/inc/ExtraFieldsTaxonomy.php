...
class ExtraFieldsTaxonomy{
    ...
    public function init(){
        add_action( $this->name.'_edit_form_fields', array( $this,'extra_fields'));
        // this adds the fields
        add_action( $this->name.'_add_form_fields',  array( $this,'extra_fields' ));
    ...
    function extra_fields($t) {
        	wp_enqueue_media();
            wp_enqueue_script( 'bibliotheque-admin-script', plugins_url( '\js\main.js', __DIR__ )  );
        ...
        $values = get_term_meta($t_id,'poids',true);
    
        // remove
        preg_match('/\]\[(.*)\]/', $field['name'], $matches);
              
        if($values && array_key_exists($matches[1],$values)){
        ...
        $html = '<div class="form form-admin">
            ...
            if ('textarea' == $field['type'] ) {
                ...
                <div class="form-check">
                    <input type="' . $field['type'] . '" name="' . strtolower($field['name']) . '" id="' . strtolower($field['name']) . '"  class="form-check-input ' . ( isset($field['class']) ? $field['class'] : '' ) . '"'.$checked.' />
                    <label for="' . strtolower($field['name']) . '" class="form-check-label">' . $field['label'] . '</label>
                </div>';
            } else {

                $html .=' <tr class="form-field">';
                $html .='<th valign="top" scope="row">';
                $html .= '<label for="' . strtolower($field['name']) . '" class="form-label components-base-control__label ">' . $field['label'] . ' :</label>';
                $html .= '</th>';
                $html .= '<td>';
                $html .= '<input type="' . $field['type'] . '" name="'. strtolower($field['name']) .'" id="' . strtolower($field['name']) . '" value="' .$value. '" /><br>';
                $html .= '</td>';
                $html .= '</tr>';
    ...
    //     preg_match('/\[(.*)\]/', $this->fields['name'], $matches);
    //    if (!isset($_POST['term_meta'][$matches[1]])){
    //         delete_term_meta(  $term_id,$matches[1] );
    //    };
